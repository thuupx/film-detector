...
TOOLS_PATH = 'tools.pkl'
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

CATEGORICAL_OUTPUT_NAMES = [
    'FilmSimulation', 'DynamicRange', 'GrainEffect',
    'ColorChromeEffect', 'ColorChromeEffectBlue', 'WhiteBalance', 'ISO', 'ExposureCompensation'
]
NUMERICAL_OUTPUT_NAMES = [
    'Highlight', 'Shadow', 'Color', 'NoiseReduction',
    'Sharpening', 'Clarity', 'WBShiftRed', 'WBShiftBlue'
]

# Default set (will be overridden dynamically per fold)
SEVERELY_IMBALANCED = set(['FilmSimulation', 'DynamicRange', 'WhiteBalance', 'ISO', 'ExposureCompensation'])

FIXED_NUM_RANGES = {
    'Highlight': (-2, 4), 'Shadow': (-2, 4), 'Color': (-4, 4), 'NoiseReduction': (-4, 4),
    'Sharpening': (-4, 4), 'Clarity': (-5, 5), 'WBShiftRed': (-9, 9), 'WBShiftBlue': (-9, 9)
}

# --- Utils ---
class FocalLoss(nn.Module):
    def __init__(self, alpha: float = 0.25, gamma: float = 2.0, reduction: str = 'mean', weight: torch.Tensor = None):
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        ce_loss = F.cross_entropy(logits, target, weight=self.weight, reduction='none')  # (B,)
...
class RecipesDataset(Dataset):
    def __init__(self, df: pd.DataFrame, encoders: Dict[str, LabelEncoder], transforms: T.Compose):
...
class MultiHeadMobileNet(nn.Module):
    def __init__(self, class_counts: Dict[str, int], num_numeric: int):
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(in_features, 1024), nn.ReLU(True), nn.Dropout(0.5),
        ...
        self.cat_heads = nn.ModuleDict({
            f'{name}': nn.Linear(128, num_classes) for name, num_classes in class_counts.items()
        })
        # Numerical head
...
def create_transforms(train: bool) -> T.Compose:
    if train:
        return T.Compose([
            ...
            T.RandomApply([T.ColorJitter(brightness=0.25, contrast=0.25, saturation=0.25, hue=0.02)], p=0.7),
            T.RandAugment(num_ops=2, magnitude=7),
            T.RandomAffine(degrees=20, translate=(0.15, 0.15), shear=12, scale=(0.85, 1.15)),
            T.ToTensor(),
...
def load_and_prepare_dataframe(json_path: str) -> Tuple[pd.DataFrame, Dict[str, LabelEncoder], Dict[str, Dict[int, float]]]:
...
def detect_severely_imbalanced(df: pd.DataFrame, encoders: Dict[str, LabelEncoder], threshold: float = 10.0) -> set:
    severe = set()
    for name in CATEGORICAL_OUTPUT_NAMES:
...
def build_weighted_sampler(df: pd.DataFrame, encoders: Dict[str, LabelEncoder], head: str = 'FilmSimulation') -> WeightedRandomSampler:
...
def train_one_fold(model: nn.Module,
                   ...
                   epochs: int = 50,
                   lr: float = 1e-4,
                   freeze_epochs: int = 5) -> Tuple[nn.Module, Dict[str, List[float]]]:
    model.to(DEVICE)
    ...
    def run_epoch(loader: DataLoader, train_mode: bool):
        ...
        with torch.set_grad_enabled(train_mode):
            for images, cat_targets, num_targets in loader:
                ...
                cat_targets_t = {k: torch.tensor(v, dtype=torch.long, device=DEVICE) for k, v in cat_targets.items()}
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} - train_loss: {train_loss:.4f} - val_loss: {val_loss:.4f} - val_mae: {val_metrics['mae']:.4f} - {head_accs}")
...
def train_with_cv(df: pd.DataFrame, encoders: Dict[str, LabelEncoder], class_weights: Dict[str, Dict[int, float]]):
    # Prepare stratify labels from FilmSimulation
    film_labels = encoders['FilmSimulation'].transform(df['FilmSimulation'].astype(str).tolist())

    # Class counts
    class_counts = {name: len(encoders[name].classes_) for name in CATEGORICAL_OUTPUT_NAMES}
    ...
    for fold, (tr_idx, va_idx) in enumerate(skf.split(indices, film_labels)):
        ...
        train_loader = DataLoader(train_ds, batch_size=12, shuffle=False, sampler=sampler, num_workers=2, pin_memory=True)
        val_loader = DataLoader(val_ds, batch_size=16, shuffle=False, num_workers=2, pin_memory=True)

        model = build_model(class_counts, num_numeric)
        # Detect severely imbalanced heads on the training subset
        severely_imbalanced = detect_severely_imbalanced(train_subset, encoders, threshold=10.0)
        model, history = train_one_fold(
            model, train_loader, val_loader, class_counts, severely_imbalanced, class_weights,
            epochs=150, lr=1e-4, freeze_epochs=5
        )
...
def main():
    ...
    best_model, history, class_counts, num_numeric = train_with_cv(df, encoders, class_weights)
