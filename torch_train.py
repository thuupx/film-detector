...
class FilmDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cat_targets = torch.tensor(
            [int(row[c]) for c in self.cat_cols], dtype=torch.long
        )
...
def make_balanced_sampler(
    df: pd.DataFrame,
    priority_cols: List[str] = ["FilmSimulation"],
    ...
    if priorities is None:
        priorities = [0.7] + [
            0.2,
            0.1,
        ]  # default: 70% film, 20% WB, 10% ISO (if present)

    # compute per-column inverse freq maps
    col_counts = {
        col: Counter(df[col].values) for col in priority_cols if col in df.columns
    }
...
class FocalLoss(nn.Module):
    def __init__(
        self, weight: torch.Tensor = None, gamma: float = 2.0, alpha: float = 1.0
    ):
        super().__init__()
        self.gamma = gamma
        # weight argument will be passed to cross_entropy inside forward
        self.register_buffer(
            "weight_buf", weight if weight is not None else torch.tensor([])
        )  # keep on-device
        self.alpha = alpha
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor):
        ...
        ce = F.cross_entropy(
            logits,
            targets,
            weight=(self.weight_buf if len(self.weight_buf) > 0 else None),
            reduction="none",
        )
        pt = torch.exp(-ce)
...
def mixed_loss(
    criterion, outputs: torch.Tensor, y_a: torch.Tensor, y_b: torch.Tensor, lam: float
):
...
# -------------------------
def load_data() -> Tuple[
    pd.DataFrame, Dict[str, object], Dict[str, torch.Tensor], Dict[str, int]
]:
    ...
    for col in CATEGORICAL_OUTPUTS:
        ...
        print(
            f"{col}: classes={len(uniques)}, counts min/max={counts.min()}/{counts.max()}"
        )
...
def train_one_epoch(
    ...
    for images, cat_targets, num_targets in loader:
        ...
        mixed_images, y_a_cat, y_b_cat, y_a_num, y_b_num, lam = mixup_fn(
            images, cat_targets, num_targets
        )
        ...
        if y_b_num is not None:
            num_l = mixed_loss(
                num_loss, num_outputs, y_a_num.to(DEVICE), y_b_num.to(DEVICE), lam
            )
        else:
    ...
    return (
        avg_loss,
        individual_losses,
    )  # return last batch losses as sample for DWA tracking
...
def validate(model, loader, cat_losses, num_loss, film_metrics, task_names):
    ...
    with torch.no_grad():
        for images, cat_targets, num_targets in loader:
            ...
            film_top5_correct += (
                (topk == film_targets.unsqueeze(1)).any(dim=1).sum().item()
            )
    ...
    aux_accuracies = {
        name: (aux_correct[name] / max(1, aux_total[name])) for name in aux_correct
    }
...
class DWA:
    ...
    def update(self, task_losses: List[float]):
        ...
        r = np.array(
            [
                (self.loss_hist[i][-1] / (self.loss_hist[i][-2] + 1e-12))
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, film_labels)):
        ...
        sampler = make_balanced_sampler(
            train_df,
            priority_cols=["FilmSimulation", "WhiteBalance", "ISO"],
            priorities=[0.7, 0.2, 0.1],
        )
        ...
        head_params = (
            ...
            + list(model.categorical_heads.parameters())
            + list(model.numerical_head.parameters())
        )
        ...
        task_weights = torch.ones(
            num_tasks, dtype=torch.float32, device=DEVICE
        )  # last pos for numerical
        ...
        for epoch in range(EPOCHS):
            ...
            val_loss, film_results, aux_accuracies = validate(
                model, val_loader, cat_losses, num_loss, film_metrics, task_names
            )
            ...
            print(
                f"FilmSimulation - Top1: {film_top1:.3f} Top5: {film_top5:.3f} MacroF1: {film_f1:.3f} BalancedAcc: {film_results['balanced_accuracy']:.3f}"
            )
            ...
            if (
                isinstance(last_task_losses, list)
                and len(last_task_losses) >= num_tasks
            ):
                # If train_one_epoch returned individual_losses as last batch, use them
                per_task_losses = np.array(
                    last_task_losses[:num_tasks], dtype=float
                ).tolist()
            else:
            ...
            if film_f1 > best_fold_f1:
                ...
                if epochs_no_improve >= PATIENCE:
                    print(
                        f"Early stopping triggered on fold {fold + 1} at epoch {epoch + 1}"
                    )
                    break
        ...
        torch.save(
            {
                ...
                "num_numeric": len(NUMERICAL_OUTPUTS),
            },
            MODEL_PATH,
        )
        tools = {
